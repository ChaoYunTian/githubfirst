...
Matrix& Matrix::operator=(const Matrix &m)  //重载赋值运算符，完成对象的拷贝
{
   if(&m != this)
   {
      height = m.height;
...
Matrix Matrix::operator+(const Matrix &m)  //两个尺寸相同的矩阵，对应元素的值相加
{
   if(height != m.height || width != m.width)
   {
      return Matrix();
...
Matrix Matrix::operator-(const Matrix &m)
{
   if(height != m.height || width != m.width)
   {
      height = (m.height > height)? height:m.height;
      width = (m.width > width)? width:m.width;
      Matrix temp(height,width);
      for(int i = 0; i < height; i++)
      {
         for(int j = 0; j < width; j++)
...
Matrix operator+(Matrix &m, double num)  //所有元素加上同一个数值
{
   Matrix temp(m);
   for(int i = 0; i < m.height; i++)
   {
